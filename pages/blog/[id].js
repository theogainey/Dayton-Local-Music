  ...
  return(
    <Layout >
      <Head>
        ...
        {postData.authorTwitter &&(<meta property="twitter:creator:id" content={`${postData.authorTwitter}`}/>)}
      </Head>
      <article>
        <h1 className="text-xl  font-bold mt-2">{postData.title}</h1>
        <div className="w-full flex flex-row items-center justify-between">
          <div className="w-auto text-left  flex flex-col items-start justify-center">
            <Link href={`/people/${postData.authorHref}`}>
              <a className="text-blue-500">{`${postData.author} `}</a>
            </Link>
            <div className="mt-0">
          ...
          <div className="w-auto flex flex-row items-center justify-end">
            <a target="_blank" className="py-4 px-4">
              <svg height="30" width="30" viewBox={"0 0 24 24"} fill={"#616161"}>
                <path d="M5 3h14a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2m13 2h-2.5A3.5 3.5 0 0 0 12 8.5V11h-2v3h2v7h3v-7h3v-3h-3V9a1 1 0 0 1 1-1h2V5z"/>
              </svg>
            </a>
            <a target="_blank" className="py-4" href={`https://twitter.com/intent/tweet?url=https://www.daytonlocalmusic.com/blog/${postData.id}`}>
              <svg height="30" width="30" viewBox={"0 0 24 24"} fill={"#616161"}>
        ...
        {postData.img &&(<div className="w-full max-h-1/3">
          <Image
            alt={postData.imgAlt}
            src={`/${postData.img}`}
            priority
